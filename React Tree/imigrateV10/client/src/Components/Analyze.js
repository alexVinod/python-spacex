...
import help_icon from "./images/help-light.png";
...
export default function Analyze() {
    ...
    const [configDetails, setConfigDetails] = useState({
        ...
        database: "N/A",
        techStack: "Java",
        sourceCodeRepo: "https://github.com/gaurav-dubey-gitspace/spring-music",
        dropletsCount: "1",
    ...
    const [gkeDetails, setGkeDetails] = useState({ projectId: "lexical-acolyte-352805", region: "us-east1-b", cluster: "jenkins-ubuntu-containerd", namespace: "devops-tools", jsonval: "" });
    ...
    const onSelectKubeFoundary = () => {
        const comboValue = document.getElementById("kubeCustomSelect").value;
        setModelUI({ condModel: comboValue });
    }

    const saveHandler = () => {
        let targetVal = document.getElementById("kubeCustomSelect").value;
        ...
        try {
            if (targetVal !== "" &&
                ...
                axios.get(`${hostname}/projectExists/${targetVal}/${gkeDetails.projectId}`, data).then((response) => {
                    ...
                    axios.post(`${hostname}/createConfigs`, dataParams).then((_response) => {
    ...
    const saveAnalyzeHandler = () => {
        try {
            ...
            if (configDetails.cfPlatform !== "" &&
                ...
                axios.post(`${hostname}/createAnalyze`, configDetails).then((_response) => {
                    console.log("DATA SAVED");
    ...
    return (
        <React.Fragment>
            ...
            <div className='container ' >
                <form method='POST'>
                    <div className='row '>
                        <div className='col-11 bg-warning mt-1 rounded-start'>
                            <h3 className='text-light mt-1 animated bounce infinite'> <img src={analyze_icon} alt="analyze_image" className='xsa-icon mt-mins-5' /> Assess </h3>
                        </div>
                        <div className='col-1 bg-warning mt-1 rounded-end'>
                    ...
                    <table className='mx-5 '>
                        <tbody>
                            <tr>
                                ...
                                <td><input type="text"
                                    className='mx-1 form-control txt-focus'
                                    onChange={(e) => setConfigDetails(
                                        { ...configDetails, cfPlatform: e.target.value })}
                                    value={configDetails.cfPlatform} autoFocus /></td>
                            </tr>
                            <tr>
